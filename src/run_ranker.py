...
class GetRecs:
    ...
    def get_prompts_hidden(self, data_dict, full=False):
        embedding_data_dict = {}
        ...
        for k,v in data_dict.items():
            ...
            if full:
                ...
                min_layer = self.max_layer - 15
            for idx, repr in enumerate(hidden_states):
                if idx >= (self.max_layer-min_layer): # Might need to adjust this to the steering layer
...
if __name__ == '__main__':
    ...
    parser.add_argument('-so', '--steered_only', action='store_true', help='Run only the steered version')
    # parser.add_argument('')
    ...
    for k, v in new_dict.items():
        ...
        output_filename = f'output_dict_{args.item_type}_{size_of_sample}_withnegs_{log_reg_type}_{args.demo}_{args.layer_to_steer}_{args.model_name}.pt'
        with open(os.path.join(output_path,output_filename), 'w') as json_file:
            json.dump(inner_dict, json_file, indent=4)
